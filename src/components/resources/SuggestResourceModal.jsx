 ...
 * User-facing modal for suggesting new resources to be reviewed by admins
 * 
...
// Helper function to map Podiatry to Orthopedic Surgery + Foot and Ankle (for suggestions)
async function mapPodiatryToFootAnkleForSuggestion() {
  try {
    // Find Orthopedic Surgery specialty
    ...
    if (!orthoSurgery) {
      throw new Error('Orthopedic Surgery specialty not found');
    }
    
    // Find Foot and Ankle subspecialty
    // Search for subspecialties that contain both "foot" and "ankle" (case-insensitive, any order/separator)
    const { data: allSubspecialties } = await supabase
      .from('subspecialties')
      .select('id, name')
      .eq('specialty_id', orthoSurgery.id);
    
    // Filter in JavaScript to find one that contains both "foot" and "ankle"
    const footAnkle = allSubspecialties?.find(sub => {
      const nameLower = sub.name.toLowerCase();
      return nameLower.includes('foot') && nameLower.includes('ankle');
    });
    
    if (footAnkle?.id) {
      console.log('✅ Found Foot & Ankle subspecialty:', { id: footAnkle.id, name: footAnkle.name });
    }
    
    if (!footAnkle) {
      // Log all subspecialties under Orthopedic Surgery for debugging
      const { data: allSubspecialties } = await supabase
        .from('subspecialties')
        .select('id, name, specialty_id')
        .eq('specialty_id', orthoSurgery.id);
      
      console.error('❌ Foot and Ankle subspecialty not found. Available subspecialties:', allSubspecialties);
      throw new Error(`Foot and Ankle subspecialty not found under Orthopedic Surgery. Available: ${allSubspecialties?.map(s => s.name).join(', ') || 'none'}`);
    }
 ...
 */
export default function SuggestResourceModal({ currentUser, onSubmit, onClose }) {
  ...
  useEffect(() => {
    if (currentUser) {
      console.log('🔵 SuggestResourceModal: Current user profile:', {
  ...
  useEffect(() => {
    if (selectedSpecialty && !isInitialLoad.current) {
      ...
      const specialty = specialties.find(s => String(s.id) === String(selectedSpecialty));
      if (specialty && specialty.name.toLowerCase().includes(SPECIALTY_SUBSPECIALTY.PODIATRY)) {
        setIsPodiatry(true);
        // Podiatry doesn't have subspecialties, so we'll map it later
        setSubspecialties([]);
        setSelectedSubspecialty(null);
        // Auto-load Foot and Ankle categories
        loadFootAnkleCategories();
      } else {
        setIsPodiatry(false);
  ...
  async function loadInitialData() {
    try {
      ...
      if (currentUser?.specialtyId) {
        // Check if user's specialty is Podiatry (by name match)
        const userSpecialty = specialtiesData?.find(s => String(s.id) === String(currentUser.specialtyId));
        console.log('🔍 Podiatry Detection:', {
          userSpecialtyId: currentUser.specialtyId,
          userSpecialty: userSpecialty?.name,
          userEmail: currentUser.email,
          allSpecialties: specialtiesData?.map(s => ({ id: s.id, name: s.name }))
        });
        const isUserPodiatry = userSpecialty && userSpecialty.name.toLowerCase().includes(SPECIALTY_SUBSPECIALTY.PODIATRY);
        
        console.log('🔍 Podiatry Check Result:', {
          isUserPodiatry,
          specialtyName: userSpecialty?.name,
          checkString: SPECIALTY_SUBSPECIALTY.PODIATRY,
          nameIncludesPodiatry: userSpecialty?.name.toLowerCase().includes(SPECIALTY_SUBSPECIALTY.PODIATRY)
        });
        
        if (isUserPodiatry) {
          // Podiatry user: Set Podiatry as specialty, don't set subspecialty (will be mapped on submit)
          console.log('✅ Podiatry user detected - setting Podiatry UI');
          setIsPodiatry(true);
          setSelectedSpecialty(String(currentUser.specialtyId));
          setSubspecialties([]);
          setSelectedSubspecialty(null);
          // Auto-load Foot and Ankle categories
          await loadFootAnkleCategories();
        } else {
          console.log('⚠️ Non-Podiatry user detected - specialty:', userSpecialty?.name);
          // Non-Podiatry user: Load subspecialties normally
  ...
  async function loadFootAnkleCategories() {
    try {
      const mapping = await mapPodiatryToFootAnkleForSuggestion();
      await loadCategories(mapping.subspecialtyId);
    } catch (error) {
      console.error('Error loading Foot and Ankle categories:', error);
      setImageError('Error loading categories for Podiatry');
    }
  }
  ...
  // The suggested category name will be stored with the resource suggestion
  // Admins will review and create the category if approved
  const handleSuggestCategory = () => {
    if (!newCategoryName.trim()) {
      toast.error('Please enter a category name');
      return;
    }
    // Just close the input and keep the suggested name
    // It will be included in the form submission
    setShowNewCategoryInput(false);
    toast.success('Category suggestion will be included with your resource suggestion. An admin will review it.');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Category validation: Either selectedCategory or newCategoryName must be provided
    if (!selectedCategory && !newCategoryName.trim()) {
      setImageError('Please select a category or suggest a new one');
      return;
    }
    ...
    const timeoutId = setTimeout(() => {
      console.error('Submission timeout (30s), resetting...');
      setSubmitting(false);
      setImageError('Submission timed out. Please try again.');
    }, 30000);
    
    try {
      let finalSpecialtyId = selectedSpecialty;
      let finalSubspecialtyId = selectedSubspecialty;
      
      // Special case: Podiatry maps to Orthopedic Surgery + Foot and Ankle
      if (isPodiatry) {
        console.log('🦶 Podiatry user detected - mapping to Orthopedic Surgery/Foot and Ankle');
        const mapping = await mapPodiatryToFootAnkleForSuggestion();
        finalSpecialtyId = mapping.specialtyId;
        finalSubspecialtyId = mapping.subspecialtyId;
        console.log('🦶 Mapping result:', {
          originalSpecialty: 'Podiatry',
          mappedSpecialtyId: finalSpecialtyId,
          mappedSubspecialtyId: finalSubspecialtyId
        });
      }
      
      // Security: Validate IDs before submission
      if (!finalSpecialtyId || !finalSubspecialtyId) {
        throw new Error('Specialty and subspecialty are required');
      }
      
      // Validate: Either category_id or suggested_category_name must be provided
      if (!selectedCategory && !newCategoryName.trim()) {
        setImageError('Please select a category or suggest a new one');
        return;
      }
        ...
        <div className="p-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600">Submit a resource for review. An admin will review it before it appears in the library.</p>
            </div>
            <button
              onClick={onClose}
              className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
              aria-label="Close suggest resource modal"
            >
              <X size={24} />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-purple-50 rounded-xl p-4 border-2 border-purple-200">
              <div className="flex items-center justify-between mb-3">
                ...
                {!isPodiatry && !canEditSpecialty && (selectedSpecialty || selectedSubspecialty) && (
                  <button
                    type="button"
              ...
              {loadingData ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Specialty: Show "Podiatry" as read-only for Podiatry users, dropdown for others */}
                  {isPodiatry ? (
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1.5">Specialty</label>
                      <div className="w-full px-3 py-2 text-sm bg-gray-100 text-gray-600 border-2 border-gray-200 rounded-lg">
                        Podiatry
                      </div>
                    </div>
                  ) : (
                  ...
                  {/* Subspecialty (hidden for Podiatry) */}
                  {!isPodiatry && (
                    <div>
            ...
            {/* Category Selection */}
            {(selectedSubspecialty || isPodiatry) && (
              <div>
                ...
                {showNewCategoryInput ? (
                  <div className="space-y-2">
                    <input
                      ...
                      onKeyPress={(e) => e.key === 'Enter' && handleSuggestCategory()}
                      placeholder="Enter suggested category name..."
                    ...
                    <p className="text-xs text-gray-500">
                      This category suggestion will be reviewed by an admin. You can still submit your resource suggestion.
                    </p>
                  ...
                  >
                    ...
                    <option value="suggest_new" className="font-semibold text-purple-600">
                      + Suggest new category
                    </option>
                  </select>
                )}
                {newCategoryName.trim() && !showNewCategoryInput && (
                  <div className="mt-2 p-2 bg-purple-50 border border-purple-200 rounded-lg">
                    <p className="text-sm text-purple-700">
                      <strong>Suggested category:</strong> {newCategoryName}
                    </p>
                    <button
                      type="button"
                      onClick={() => {
