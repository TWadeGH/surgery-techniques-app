// Microsoft Outlook OAuth Callback - Edge Function
// Handles OAuth callback from Microsoft, exchanges code for tokens, stores in database
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('MICROSOFT_CLIENT_ID exists:', !!MICROSOFT_CLIENT_ID)
    ...
    const tokenResponse = await fetch('https://login.microsoftonline.com/common/oauth2/v2.0/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${SUPABASE_URL}/functions/v1/outlook-oauth-callback`,
    ...
    // Get user's default calendar via Microsoft Graph API
    ...
    const primaryCalendar = calendars.find((cal: any) => cal.isDefaultCalendar === true)
    ...
    console.log('Default calendar found:', primaryCalendar.id)

    // Get user email from Microsoft Graph
    const userResponse = await fetch('https://graph.microsoft.com/v1.0/me?$select=mail,userPrincipalName', {
      headers: { Authorization: `Bearer ${access_token}` }
    })

    let calendarEmail = primaryCalendar.id
    if (userResponse.ok) {
      const userProfile = await userResponse.json()
      calendarEmail = userProfile.mail || userProfile.userPrincipalName || primaryCalendar.id
    }
    ...
    const { error: dbError } = await supabase
      ...
      .upsert({
        ...
        calendar_name: primaryCalendar.name || primaryCalendar.id,
    ...
    return new Response(null, {
      ...
      headers: {
        ...corsHeaders,
        'Location': `${APP_URL}/settings?calendar=connected&provider=microsoft`
      }
    })
